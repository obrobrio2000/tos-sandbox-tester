#!/usr/bin/env bash
set -euo pipefail

# replace with URL generated by ngrok
URL='YOUR_URL_HERE' # e.g. https://1234567890.ngrok.io (callback URL will be available at /translation/delivery)

SRC_LANG=en-US
DST_LANG=it-IT
POLL_INTERVAL=10      # seconds
MAX_WAIT=300          # 5 min

echo "🔧 URL: $URL"
echo "🆕 Creating order…"

ORDER_JSON=$(curl -s -X POST "$URL/orders" \
  -H 'Content-Type: application/json' \
  -d "{\"name\":\"Test $(date +%T)\",\"sourceLang\":\"$SRC_LANG\",\"targetLang\":\"$DST_LANG\"}")
ORDER_ID=$(echo "$ORDER_JSON" | jq -r '.id')
echo "✅ Order id=$ORDER_ID"

echo "➕ Adding texts…"
curl -s -X POST "$URL/orders/$ORDER_ID/texts" \
  -H 'Content-Type: application/json' \
  -d '{"content":"Hello world!"}'  > /dev/null
curl -s -X POST "$URL/orders/$ORDER_ID/texts" \
  -H 'Content-Type: application/json' \
  -d '{"content":"How are you today?"}' > /dev/null
echo "✅ Added 2"

echo "🚀 Submitting…"
curl -s -X POST "$URL/orders/$ORDER_ID/submit" > /dev/null
echo "✅ Submitted"

echo -n "⏳ Waiting for delivery"; echo
ELAPSED=0
while (( ELAPSED < MAX_WAIT )); do
  ORDER=$(curl -s "$URL/orders/$ORDER_ID")
  DELIVERED=$(echo "$ORDER" | jq '[.texts[].status]|all(.=="delivered")')
  if [[ "$DELIVERED" == "true" ]]; then
    echo; echo "🎉 Delivered!"
    break
  fi
  STATUSES=$(echo "$ORDER" | jq -r '[.texts[].status]|join(",")')
  echo -n "($STATUSES)"; echo
  sleep "$POLL_INTERVAL"
  (( ELAPSED += POLL_INTERVAL ))
done

if (( ELAPSED >= MAX_WAIT )); then
  echo; echo "⚠️  Timeout after $MAX_WAIT s:"
  echo "$ORDER" | jq '.texts[] | {id,status}'
  exit 1
fi

echo; echo "📦 Final payload:"
echo "$ORDER" | jq '.texts[] | select(.content != null) | {id,orderId,content,translatedContent,status}'

echo "🗑️  Cleaning up…"
curl -s -X DELETE "$URL/orders/$ORDER_ID" > /dev/null
echo "✅ Deleted order $ORDER_ID"
echo "🗑️  Cleaning up texts…"
curl -s -X DELETE "$URL/orders/$ORDER_ID/texts" > /dev/null
echo "✅ Deleted texts"

echo; echo "✅ All done!"